    ...
    try {
      await db.llmCall.create({
        data: {
          ...
          cacheCreationInputTokens:  (response.usage as Record<string, number>)['cache_creation_input_tokens'] ?? 0,
          cacheReadInputTokens:      (response.usage as Record<string, number>)['cache_read_input_tokens'] ?? 0,
          costCents,
          batched: false,
        },
      });
