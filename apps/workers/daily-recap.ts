...
async function generateUserRecap(
  ...
  // d/e. Route through the shared LLM router — it resolves the model, writes
  // the `llm_calls` audit row and debits the token ledger. A premium user with
  // an exhausted balance is transparently downgraded to Haiku rather than
  // driving the ledger further negative.
